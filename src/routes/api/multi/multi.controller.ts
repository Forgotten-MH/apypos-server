...
import Room, { IRoom } from "../../../model/room.js";
...
export const roomReserve = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomReserveInput;
    const filter = { current_session: body.session_id };
    ...
    const roomData = {
      ...
      room_name: body.room_name || `${user.character_name}的房间`,
      quest_id: body.quest_id,
      quest_name: body.quest_name || "",
      auto_flag: body.auto_flag || 0,
      quick_match: body.quick_match || 0,
      kick: body.kick || 0,
      restart: body.restart || 0,
      tag: body.tag || 0,
...
export const roomReserveJoin = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomReserveJoinInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    const room = await Room.findOne({ room_id: body.room_id });
    if (!room) {
...
export const roomSearch = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomSearchInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    console.log(`[Room Search] User ${user.user_id} searching rooms with params:`, body);

    const searchConditions: mongoose.QueryFilter<IRoom> = {
      phase: 0,
      is_locked: false,
      is_private: false,
      ...(body.quest_id ? { quest_id: body.quest_id } : {}),
    };
...
export const roomJoin = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomJoinInput;
    console.log(`[Room Join] Request received - session_id: ${body.session_id}, room_id: ${body.room_id}`);

    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      console.log(`[Room Join] User not found for session: ${body.session_id}`);
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    }
    ...
    const room = await Room.findOne({ room_id: body.room_id });
    if (!room) {
      console.log(`[Room Join] Room not found: ${body.room_id}`);
      return encryptAndSend({}, res, req, 4002); // Room not found
    }
...
export const roomQuick = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomQuickInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    if (!room) {
      const roomData = {
        host_id: user.user_id || "",
        host_name: user.character_name || "",
        room_name: body.name || `${user.character_name}的快速房间`,
        quest_id: body.quest_id || 0,
        auto_flag: body.auto_flag || 0,
        quick_match: 1,
        kick: body.kick || 0,
        restart: body.restart || 0,
...
export const roomGet = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomGetInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    if (body.room_id) {
      room = await Room.findOne({ room_id: body.room_id });
    }
...
export const roomCreate = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomCreateInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    const roomData = {
      host_id: user.user_id || "",
      host_name: user.character_name || "",
      room_name: body.name || `${user.character_name}的房间`,
      quest_id: body.quest_id,
      quest_name: body.quest_name || "",
      auto_flag: body.auto_flag || 0,
      quick_match: body.quick_match || 0,
...
export const inviteList = async (req: Request, res: Response) => {
  try {
    const body = req.body as SessionOnlyInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
...
export const roomLock = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomLockInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
...
export const roomKick = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomKickInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
    ...
    const targetMember = room.members.find((member) => member.user_id === target_user_id);
    if (!targetMember) {
...
export const roomReady = async (req: Request, res: Response) => {
  try {
    const body = req.body as RoomReadyInput;
    const filter = { current_session: body.session_id };
    const user = await User.findOne(filter);
    if (!user) {
      return encryptAndSend({}, res, req, 2004); // Not authenticated
...
export const memberInfo = async (req: Request, res: Response) => {
  try {
    const body = req.body as Partial<MemberInfoInput>;
    const session_id = body.session_id || (typeof req.query.session_id === "string" ? req.query.session_id : "");
    const sequence = body.sequence !== undefined ? body.sequence : Number(req.query.sequence);
    ...
    console.log(`[MemberInfo] Room members:`, room.members.map((m) => ({
      user_id: m.user_id,
      character_name: m.character_name,
    })));

    const memberUserIds = room.members.map((member) => member.user_id);
    ...
    const playerDetails = room.members.map((member, index: number) => {
      ...
      const selectedEquipSet = userInfo.equipset?.equip_sets?.find(
        (set) => set.index === selectedEquipSetIndex
      ...
      const getEquipHash = (equipmentId: string | undefined, slotName: string): number => {
        ...
        const equipment = userInfo.box?.equipments?.find((e) => e.equipment_id === equipmentId);
      ...
      const socialEquipSet = userInfo.social_equip_sets?.[0]?.knight;
    ...
    console.log(`[MemberInfo] Player details summary:`, playerDetails.map((p) => ({
      user_id: p.user_id,
      name: p.name,
    ...
    const data = {
      free: [],
      group: [],
      order: room.members.map((m) => m.user_id),
      phase: room.phase,
